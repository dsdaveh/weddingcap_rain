...
source( "../team/data-prep.R")  ; tcheck(desc = "load dataset")

cat("loading train from CSV\n")
train <- fread("../train.csv")  ; tcheck(desc = "load dataset from csv")
#reducing size for test run - JAMES EDITS
train_1 <- train
train_1 <- subset(train_1, Id <= 25000)
#train_50 <- subset(train, Id > 250000 & Id <= 500000)
#train_75 <- subset(train, Id > 500000 & Id <= 750000)
#train_90 <- subset(train, Id > 750000 & Id <= 900000)
#train_100 <- subset(train, Id > 900000 & Id <= 950000)
#train_110 <- subset(train, Id > 950000 & Id <= 1000000)
#train_120 <- subset(train, Id > 1000000 & Id <= 1050000)
#train_130 <- subset(train, Id > 1050000 & Id <= 1100000)
#train_140 <- subset(train, Id > 1100000 & Id <= 1150000)
#train_150 <- subset(train, Id > 1150000 & Id <= 1200000)

setkey(train_1, Id, minutes_past)

tcheck(0)

impute_steps_train <- data.frame( Id = unique(train_1$Id)) %>% arrange( Id ) %>% tbl_df

#Id's with first = 0  
has_t0 <- train_1[ minutes_past == 0, ]$Id
add_t0  <- train_1[ ! Id %in% has_t0, .SD[1],  Id]
add_t60 <- train_1[                 , .SD[.N], Id]   #no Id's have last = 60
add_t0$minutes_past  <- 0
add_t60$minutes_past <- 60

impute_steps_train$add_t0 <- ! impute_steps_train$Id %in% has_t0

train_1 <- rbind( train_1, add_t0, add_t60);   tcheck(desc = "add t0 and t60")
...
# James Edits - Horizontal Imputation Method
#train_noNAs <- filter(train, (Ref != "NA"))
train_noNARefs_select_fields <- select(train_1, minutes_past, 
                                       radardist_km,Ref, RefComposite, RhoHV, 
                                       RhoHV_5x5_10th, Zdr, Zdr_5x5_10th,
                                       Kdp, Kdp_5x5_90th, Expected)
imp_trainNoNARefs <- mice(data = train_noNARefs_select_fields, print = FALSE, maxit =2, seed = 2)
train_noNARefs_imputed <- complete(imp_trainNoNARefs, action = 3)
train_noNARefs_imputed$Id <- train_1$Id
train_1 <- train_noNARefs_imputed
...
#Reducing test size -JAMES EDITS
test <- test
...
#James Edits
test_noNARefs_select_fields <- select(test, minutes_past, 
                                       radardist_km,Ref, RefComposite, RhoHV, 
                                       RhoHV_5x5_10th, Zdr, Zdr_5x5_10th,
                                       Kdp, Kdp_5x5_90th)
imp_testNoNARefs <- mice(data = test_noNARefs_select_fields, print = FALSE, maxit =2, seed = 2)
test_noNARefs_imputed <- complete(imp_testNoNARefs, action = 3)
test_noNARefs_imputed$Id <- test$Id
test <- test_noNARefs_imputed
