...
Regardless of which route we take, we need to decide on the proper theshold.  The plot below zooms in on the cumulative err density for the highest measured values.  The measured (y) and predicted (y') are shown shown at cum err = 10%, 20%, 30% and 40%.  We can see that y and y' are well coordinated at 10% ( y = Expected ~ `r y_10p` mm), but at higher values the correlation doesn't hold.  This suggests the threshold we should use is around this value.  The MAE for measured observations below this value is `r mae_cut10p`, and `r mae_cut10px` above it.
