...
#remove NA's   ## in two parts, since my laptop runs out of RAM
cpt <- floor(nrow(train)/2) #cutpoint
object.size(train)
na_obs_1 <- train[ 1:cpt, ] %>% 
    select( starts_with("Ref"), starts_with("Rho"), starts_with("Zdr"), starts_with("Kdp")) %>%
    .[, is.na(.SD)] %>% 
    rowSums() == 20
tcheck()
na_obs_2 <- train[ (cpt+1):nrow(train), ]%>% 
    select( starts_with("Ref"), starts_with("Rho"), starts_with("Zdr"), starts_with("Kdp")) %>%
    .[, is.na(.SD)] %>% 
    rowSums() == 20
tcheck()
na_obs <- c( na_obs_1, na_obs_2)
